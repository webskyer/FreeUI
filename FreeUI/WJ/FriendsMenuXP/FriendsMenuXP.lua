local F, C, L, _ = unpack(select(2, ...))
------------ FriendsMenuXP skin ------------
local frames = CreateFrame("Frame")
--local SecureActionButton_OnClick = SecureActionButton_OnClick
frames:RegisterEvent("PLAYER_LOGIN")
frames:SetScript("OnEvent", function(self, event, addon)
	if not IsAddOnLoaded("FriendsMenuXP") then 
		FriendsMenuXPMenuBackdrop:StripTextures() 
		--FriendsMenuXPMenuBackdrop:SetFrameStrata("BACKGROUND")
	else return 
	end
	if not IsAddOnLoaded("FriendsMenuXPSecure") then 
...
FriendsMenuXP_Buttons["ARMORY"] = {
    ...
    func = function(name)
		...
		local host = portal == 'tw' and "http://www.battlenet.com/" or ("http://tw.battle.net/"):format(GetCVar'portal')
...
function FriendsMenuXP_OnLoad(self)
    ...
    if(FRIENDS_MENU_XP_LOADED) then  end
end
