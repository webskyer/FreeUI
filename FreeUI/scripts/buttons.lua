...
local function updateHotkey(self)
	...
	if showHotKey then
		...
		if text then
			...
			text = text:gsub("(a%-)", "A")
			text = text:gsub("(c%-)", "C")
			text = text:gsub(text, "(Mouse Button )", "M")
			text = text:gsub(text, KEY_BUTTON3, "M3")
			text = text:gsub(text, KEY_PAGEUP, "PU")
			text = text:gsub(text, KEY_PAGEDOWN, "PD")
			text = text:gsub(text, KEY_SPACE, "SpB")
			text = text:gsub(text, KEY_INSERT, "Ins")
			text = text:gsub(text, KEY_HOME, "Hm")
			text = text:gsub(text, KEY_DELETE, "Del")
			text = text:gsub(text, KEY_NUMPADDECIMAL, "Nu.")
			text = text:gsub(text, KEY_NUMPADDIVIDE, "Nu/")
			text = text:gsub(text, KEY_NUMPADMINUS, "Nu-")
			text = text:gsub(text, KEY_NUMPADMULTIPLY, "Nu*")
			text = text:gsub(text, KEY_NUMPADPLUS, "Nu+")
			text = text:gsub(text, KEY_NUMLOCK, "NuL")
			text = text:gsub(text, KEY_MOUSEWHEELDOWN, "MWD")
			text = text:gsub(text, KEY_MOUSEWHEELUP, "MWU")
